...
const inputGrafoDesenhado = document.getElementById(
	"inputGrafoDesenhado"
);
...
const atualizarGrafo = () => {
	...
	listaDeConexoes.every((elemento) => {
		...
		if (origem > 0 && destino > 0) {
			...
			if (dirigido) {
				grafoDesenhado.addEdge(numeroParaLetra(origem), numeroParaLetra(destino), {
					directed: true
				})
			} else {
				grafoDesenhado.addEdge(numeroParaLetra(origem), numeroParaLetra(destino))
...
const printarGrafo = () => {
	...
	var renderer = new Graph.Renderer.Raphael(inputGrafoDesenhado, grafoDesenhado, 400, 300);
	renderer.draw();
};
